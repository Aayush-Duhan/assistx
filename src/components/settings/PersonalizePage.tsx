...
export const PersonalizePage = observer(() => {
	...
	const filtered = useMemo(() => {
 		...
 		return contexts.filter((c) =>
 			c.title.toLowerCase().includes(normalizedQuery) || (c.content || '').toLowerCase().includes(normalizedQuery)
 		);
 	}, [contexts, normalizedQuery]);

	const favoriteContexts = useMemo(() => filtered.filter((c) => userContextStore.isFavorite(c.id)), [filtered]);
	const otherContexts = useMemo(() => filtered.filter((c) => !userContextStore.isFavorite(c.id)), [filtered]);

	const RoleCard = ({ ctx, onClick }: { ctx: ReturnType<typeof userContextStore.getAllContexts>[number]; onClick: () => void; }) => {
 		...
 		return (
 			<div className={cn(
 				...
 				<div className="flex items-start justify-between">
 					...
 					<button
 						className={cn("w-6 h-6 shrink-0 rounded hover:bg-white/10 flex items-center justify-center", isFav ? "text-yellow-300" : "text-white/70")}
 						onClick={() => userContextStore.toggleFavorite(ctx.id)}
 						title={isFav ? "Unpin" : "Pin to Favorites"}
 					>
 						<svg viewBox="0 0 24 24" fill="currentColor" className={cn("w-4 h-4", isFav ? "" : "opacity-60")}> 
 				...
 				<div className="mt-3 flex items-center justify-between">
 					{isActive ? (
 						<span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/80">Active</span>
 					) : (
 						<span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/50">Inactive</span>
 					)}
 					<HeadlessButton
 						className={cn(
 			...
 			<div className="flex-1 min-h-0 overflow-y-auto space-y-8 pr-1 custom-scrollbar">
 					...
 					<div className="space-y-3">
 						...
 						<div className="grid grid-cols-2 gap-4">
 							{favoriteContexts.map((ctx) => (
 								<RoleCard key={ctx.id} ctx={ctx} onClick={() => userContextStore.setSelectedContext(ctx.id)} />
 							))}
 						</div>
 					</div>
 				)}
 				...
 				<div className="space-y-3">
 					<div className="flex items-center justify-between">
 						...
 						<span className="text-xs text-white/40">{otherContexts.length + favoriteContexts.length}</span>
 					</div>
 					<div className="grid grid-cols-2 gap-4">
 				...
 				<div className="space-y-3">
 					<div className="flex items-center justify-between gap-3">
 						{isRenaming ? (
 							...
 							<h2 className="text-white text-lg font-semibold cursor-text" onDoubleClick={() => setIsRenaming(true)}>
 						...
 						<div className="flex items-center gap-2">
 							<HeadlessButton
 								className={cn(
 									...
 									userContextStore.isFavorite(userContextStore.selectedContextId) ? "text-yellow-300" : "text-white/70 hover:bg-white/5"
 								)}
 								onClick={() => userContextStore.toggleFavorite(userContextStore.selectedContextId)}
 								title={userContextStore.isFavorite(userContextStore.selectedContextId) ? 'Unpin' : 'Pin to Favorites'}
 							>
 							...
 							{userContextStore.isContextActive(userContextStore.selectedContextId) ? (
 								...
 								<SettingsButton onClick={() => userContextStore.setActiveContext(userContextStore.selectedContextId)}>Activate</SettingsButton>
 							)}
 							{userContextStore.isSelectedPredefined() && userContextStore.hasOverrideForSelected() && (
 								<SettingsButton onClick={() => userContextStore.resetSelectedPredefined()}>Reset</SettingsButton>
 							)}
 							{!userContextStore.isSelectedPredefined() && (
 								<SettingsButton onClick={() => userContextStore.deleteSelectedContext()}>Delete</SettingsButton>
 							)}
 						</div>
 					</div>
