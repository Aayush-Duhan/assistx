...
function createSelectionState() {
    ...
    return {
        ...
        select: (mode: OnboardingMode) => { selectedMode = mode; },
        clear: () => { selectedMode = undefined; },
    };
}
...
describe("Mode Page", () => {
    ...
    describe("Property 2: Single selection invariant", () => {
        ...
        it("sequence of selections always results in last selection being active", () => {
            fc.assert(
                fc.property(
                    fc.array(onboardingModeArb, { minLength: 1, maxLength: 10 }),
                    (selections) => {
                        const state = createSelectionState();
    ...
    describe("Property 3: Button state reflects selection", () => {
        ...
        it("button state correctly reflects selection state transitions", () => {
            fc.assert(
                fc.property(
                    fc.array(fc.oneof(onboardingModeArb, fc.constant(undefined)), { minLength: 1, maxLength: 10 }),
                    (states) => {
