    ...
    <OnboardingPage className={isDarkMode ? "bg-[#0f0f0f]" : "bg-[#EDEEF2]"}>
      <OnboardingForm className={`justify-center ${isDarkMode ? "bg-[#0f0f0f]" : ""}`}>
        <div className="flex flex-col gap-12 items-center w-full">
          <h1
            className={`text-4xl tracking-[-1.254px] text-center font-medium ${isDarkMode ? "text-[#F0F0F0]" : "ob3-secondary-header"}`}
          >
            ...
            {MODES.map((mode, index) => (
              <React.Fragment key={mode.key}>
                ...
                {index < MODES.length - 1 && (
                  <div
                    className={`h-px ml-13 mr-3 ${isDarkMode ? "bg-white/10" : "bg-black/10"}`}
                  />
                )}
              </React.Fragment>
            ))}
      ...
      <OnboardingDemo className="h-full justify-center" isDarkMode={isDarkMode}>
        ...
        <OnboardingDescription>
          <h2 className="-translate-y-12">
            <span
              className={`text-2xl mb-4 w-[400px] inline-block ${isDarkMode ? "text-[#A0A0A0]" : "ob3-secondary-header"}`}
            >
...
export function ModeItem({
  ...
  return (
    <HeadlessButton
      onClick={onSelect}
      className={`flex items-center justify-between group relative transition-colors duration-75 rounded-xl p-2 cursor-pointer ${
        isDarkMode ? "hover:bg-[#2a2a2a]" : "hover:bg-[#F6F6F6]"
      }`}
    >
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col text-left gap-1">
          ...
          <p className={`text-sm ${isDarkMode ? "text-[#888888]" : "text-[#A3A3A5]"}`}>
            {description}
          </p>
        </div>
      </div>
