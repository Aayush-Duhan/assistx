...
const NodeConfigPanel = ({ node, onUpdate, onDelete, onClose }: NodeConfigPanelProps) => {
  ...
  useEffect(() => {
    ...
    setHeadersText(
      typeof nodeConfig.headers === "object" ? JSON.stringify(nodeConfig.headers, null, 2) : "",
    );
  ...
  const createJsonBlurHandler = (getText: () => string, onValid: (parsed: unknown) => void) => {
    return () => {
      try {
        ...
        {kind === NodeKind.Condition && (
          <div className="space-y-4 pt-2 border-t border-zinc-800">
            ...
            <p className="text-xs text-zinc-500">
              Define branches in the advanced JSON config below. Each branch can have multiple
              conditions.
            </p>
          </div>
        )}
