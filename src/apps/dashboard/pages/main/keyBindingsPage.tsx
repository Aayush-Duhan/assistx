...
import { DEFAULT_KEYBINDINGS, type Keybindings, type KeybindingsDisabled } from '@/shared/sharedState';

// Keybinding metadata with labels and categories
const KEYBINDING_CONFIG: Record<keyof Keybindings, { label: string; description: string; category: string }> = {
    start_over: { label: 'Start Over', description: 'Reset the current conversation', category: 'Actions' },
    trigger_ai: { label: 'Trigger AI', description: 'Send message to AI', category: 'Actions' },
    hide: { label: 'Hide Window', description: 'Toggle window visibility', category: 'Window Control' },
    move_window_up: { label: 'Move Up', description: 'Move window up on screen', category: 'Window Control' },
    move_window_down: { label: 'Move Down', description: 'Move window down on screen', category: 'Window Control' },
    move_window_left: { label: 'Move Left', description: 'Move window left on screen', category: 'Window Control' },
    move_window_right: { label: 'Move Right', description: 'Move window right on screen', category: 'Window Control' },
    scroll_response_up: { label: 'Scroll Up', description: 'Scroll response content up', category: 'Navigation' },
    scroll_response_down: { label: 'Scroll Down', description: 'Scroll response content down', category: 'Navigation' },
};
...
const KeybindingsPage = () => {
    ...
    const checkConflict = useCallback((accelerator: string, excludeKey: KeybindingKey): KeybindingKey | null => {
    ...
    const toggleEnabled = useCallback((key: KeybindingKey) => {
    ...
    const resetToDefault = useCallback((key: KeybindingKey) => {
                                            ...
                                            >
                                                <div
                                                    className={cn(
                                                        ...
                                                        isDisabled ? "left-0.5 translate-x-0" : "left-[calc(100%-1.125rem)] translate-x-0"
                                                    )}
                                                >
                    ...
                    <div className="relative z-10 w-full max-w-md mx-4 p-6 rounded-2xl bg-zinc-900 border border-zinc-700/50 shadow-2xl">
                        ...
                        {recordingWarning && (
                            <div className={cn(
                                ...
                                <p className={cn(
                                    "text-xs",
                                    conflictKey ? "text-red-300" : "text-amber-300"
                                )}>
                                    {recordingWarning}
                                </p>
                            </div>
