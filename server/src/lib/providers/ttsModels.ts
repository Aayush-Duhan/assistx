...
const v = (...keys: string[]) => keys.map((k) => ({ ...VOICES[k], type: "tts" }));
...
export const TTS_MODELS_CONFIG: Record<string, any> = {
  openai: {
    models: [
...
export function buildTtsProviderModels(): Record<string, any> {
