...
export function capabilitiesFromServiceKind(kind: string) {
  return (SERVICE_KIND_CAPABILITIES as any)[kind] || null;
}
 ...
 */
export function getCapabilitiesForModel(provider: string | null | undefined, model: string): ModelCapabilities {
  if (!model) return { ...DEFAULT_CAPABILITIES } as ModelCapabilities;
  ...
  if (provider) {
    const providerCaps = (PROVIDER_CAPABILITIES as any)[provider];
    if (providerCaps?.[model]) return { ...DEFAULT_CAPABILITIES, ...providerCaps[model] } as ModelCapabilities;
    if (providerCaps?.[baseModel]) return { ...DEFAULT_CAPABILITIES, ...providerCaps[baseModel] } as ModelCapabilities;
  }

  // 2. Canonical exact
  if ((MODEL_CAPABILITIES as any)[baseModel]) return { ...DEFAULT_CAPABILITIES, ...(MODEL_CAPABILITIES as any)[baseModel] } as ModelCapabilities;
  if ((MODEL_CAPABILITIES as any)[model]) return { ...DEFAULT_CAPABILITIES, ...(MODEL_CAPABILITIES as any)[model] } as ModelCapabilities;

  // 3. Pattern match (first match wins)
  for (const { pattern, caps } of PATTERN_CAPABILITIES) {
    if (matchPattern(pattern, baseModel) || matchPattern(pattern, model)) {
      return { ...DEFAULT_CAPABILITIES, ...caps } as ModelCapabilities;
    }
  }

  // 4. Floor
  return { ...DEFAULT_CAPABILITIES } as ModelCapabilities;
}
