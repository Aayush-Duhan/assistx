...
export function getCapabilitiesForModel(
  ...
  if (provider) {
    ...
    if (providerCaps?.[model])
      return { ...DEFAULT_CAPABILITIES, ...providerCaps[model] } as ModelCapabilities;
    if (providerCaps?.[baseModel])
      return { ...DEFAULT_CAPABILITIES, ...providerCaps[baseModel] } as ModelCapabilities;
  }

  // 2. Canonical exact
  if ((MODEL_CAPABILITIES as any)[baseModel])
    return {
      ...DEFAULT_CAPABILITIES,
      ...(MODEL_CAPABILITIES as any)[baseModel],
    } as ModelCapabilities;
  if ((MODEL_CAPABILITIES as any)[model])
    return { ...DEFAULT_CAPABILITIES, ...(MODEL_CAPABILITIES as any)[model] } as ModelCapabilities;
