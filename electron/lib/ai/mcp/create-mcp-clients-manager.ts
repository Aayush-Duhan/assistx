...
import { errorToString, generateUUID, Locker, safeJSONParse, toAny } from "../../utils";
...
export class MCPClientsManager {
            ...
            (bcc, tool) => {
              return {
                ...
                [createMCPToolId(clientName, tool.name)]: VercelAIMcpToolTag.create({
                  ...
                  _mcpServerId: id,
                  execute: (params: any, options: ToolCallOptions) => {
  ...
  async toolCall(id: string, toolName: string, input: unknown) {
    return safe(() => this.getClient(id))
      .map((client: any) => {
        if (!client) throw new Error(`MCP client with ID '${id}' not found`);
        return client.client;
      })
      .map((client: any) => client.callTool(toolName, input))
      .map((res: any) => {
