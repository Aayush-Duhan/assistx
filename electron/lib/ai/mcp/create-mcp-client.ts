...
export class MCPClient {
  ...
  async finishAuth(code: string, state: string) {
    if (!isMaybeRemoteConfig(this.serverConfig))
      throw new Error("OAuth authentication is only supported for remote MCP servers");
      ...
      this.oauthProvider = new PgOAuthClientProvider({
        ...
        onRedirectToAuthorization: async (authorizationUrl: URL) => {
          ...
          try {
            ...
            this.logger.warn("Failed to open system browser for OAuth", e);
          }
  ...
  async connect(oauthState?: string): Promise<Client | undefined> {
    ...
    try {
      ...
      if (this.authorizationUrl) {
        this.logger.info("Awaiting OAuth authorization via custom protocol callback");
        this.client = client;
        this.isConnected = false;
      } else {
        this.logger.info(
  ...
  private ensureProtocolSubscription() {
    if (this.unsubscribeProtocol) return;
    this.unsubscribeProtocol = onMcpOAuthCallback(async ({ code, state }) => {
      try {
        await this.ensureOAuthState(state);
        if (this.status !== "authorizing") return;
        await this.finishAuth(code, state);
        await this.connect(state);
      } catch (e) {
        this.logger.error("OAuth callback handling failed", e);
      }
    });
  }
}
